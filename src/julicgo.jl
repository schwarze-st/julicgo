...
mutable struct Problem{nx,nt,T}
  ...
  g      :: Union{Tuple{Vararg{Function}}, Function, Nothing}    # Tuple of functions (g_1,...,g_p) or function g -> [v_1,...,v_p]
...
function width(box::Box_Node{nx,nt,T}) where {nx,nt,T}
    ...
    @inbounds for i in eachindex(box.xbox)
        w = diam(box.xbox[i])
        if w > max_width; max_width = w; end
    end
...
@inline function omega(g_obj::Tuple{Vararg{Function}}, 
    x::SVector{nx,T}, t::SVector{nt,T}) where {nx,nt,T<:Real}
    max_val = -Inf
    @inbounds for g_i in g_obj
        val = g_i(x,t)
...
@inline function omega(g_obj::Function, 
    x::SVector{nx,T}, t::SVector{nt,T}) where {nx,nt,T<:Real}
    return max(g_obj(x,t))
end

@inline function bounding_omega(g_obj::Tuple{Vararg{Function}},
    x_comp::AbstractVector{nx}, tbox::SVector{nt, Interval{T}}
    ) where {nx,nt,T}
    lo = -Inf; hi = -Inf
    @inbounds for g_i in g_obj
        iv = g_i(x_comp, tbox)          
...
_iterable(r) = r isa AbstractArray ? r : (r,)

@inline function bounding_omega(g_obj::Function,
    x_comp::AbstractVector{nx}, tbox::SVector{nt, Interval{T}}
    ) where {nx,nt,T}
    lo = -Inf; hi = -Inf
    for iv in _iterable(g_obj(x_comp, tbox))
        v_lo = inf(iv); v_hi = sup(iv)
        if v_lo > lo; lo = v_lo; end  
        if v_hi > hi; hi = v_hi; end
    end
    return lo, hi
...
@inline function bounding_f(f::Function, xbox::SVector{nx, Interval{T}}, 
    tbox::SVector{nt, Interval{T}}) where {nx,nt,T}
    interval = f(xbox, tbox)
    return inf(interval), sup(interval)
end

@inline function bounding_f(f::Function, x::MVector{nx,T}, tbox::SVector{nt, Interval{T}}) where {nx,nt,T<:Real}
    interval = f(x, tbox)
    return inf(interval), sup(interval)
end
...
function initialize_W(P::Problem{nx,nt,T}) where {nx,nt,T}
    ...
    first_node.l_omega, first_node.u_omega = bounding_omega(P.g, x_comp, t_comp)
    first_node.l_f, first_node.u_f = bounding_f(P.f, x_comp, t_comp)
...
function improvementfunction_lb(P::Problem{nx,nt,T}, node::Box_Node{nx,nt,T}, current_node::Box_Node{nx,nt,T}, epsilon::Number) where {nx,nt,T}
    left = bounding_omega(P.g, node.xbox, current_node.tbox)[1]
    right = bounding_f(P.f, node.xbox, current_node.tbox)[1] - current_node.u_f
...
function p_icgo(P::Problem{nx,nt,T}, epsilon::Number=0.2, delta::Number=0.1, maxiter::Number=100000, maxtime::Number=360, minwidth::Number=1e-3,
                ...
                if current_node.u_omega < delta && bestval_check >= 0
                    ...
                    if b_hat_node !== current_node && width(b_hat_node) > width(current_node)*0.1
                        ...
                        @inbounds for child in b_hat_children
                            child.l_omega, child.u_omega = bounding_omega(P.g, child.xbox, child.tbox)
                            child.l_f, child.u_f = bounding_f(P.f, child.xbox, child.tbox)
